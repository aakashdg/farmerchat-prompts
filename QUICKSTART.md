...
Get started with `farmerchat-prompts` in 5 minutes!
...
```python
# Get crop recommendation prompt for OpenAI
prompt = manager.get_prompt("openai", "crop_recommendation")
```

### 3. Use with Your AI Provider
...
# Prepare input
full_prompt = prompt.get_full_prompt("My soil pH is 5.2. What amendments do I need?")
...
print(use_cases)
# ['crop_recommendation', 'pest_management', 'soil_analysis', 
#  'weather_advisory', 'market_insights']
```

## Exploring Prompts

```python
# Get all OpenAI prompts
openai_prompts = manager.get_prompts_by_provider("openai")

# Get all crop recommendation prompts (across providers)
...
- Check out `examples/usage_examples.py` for more detailed examples
- Read the full documentation in `README.md`
- See `PUBLISHING.md` if you want to contribute
...
- Read the full documentation in `README.md`
- See `MULTI_DOMAIN_MIGRATION.md` for architecture details
