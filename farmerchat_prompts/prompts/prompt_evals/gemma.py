"""
Gemma-optimized prompts for prompt evaluation use cases
Following Gemma's prompt engineering guidelines for agricultural fact evaluation
"""

from ...models import Prompt, PromptMetadata, Provider, UseCase, Domain

# Specificity Evaluator
GEMMA_SPECIFICITY_EVALUATOR = Prompt(
...
GEMMA_CONTRADICTION_DETECTOR = Prompt(
    metadata=PromptMetadata(
        provider=Provider.GEMMA,
...
# Relevance Evaluator
GEMMA_RELEVANCE_EVALUATOR = Prompt(
    metadata=PromptMetadata(
        provider=Provider.GEMMA,
...
GEMMA_CONVERSATIONALITY_EVAL_FOR_STITCHING = Prompt(
    metadata=PromptMetadata(
        provider=Provider.GEMMA,
...
GEMMA_PROMPT_EVALS_PROMPTS = [
    GEMMA_SPECIFICITY_EVALUATOR,
    GEMMA_FACT_GENERATOR,
    GEMMA_FACT_RECALL,
    GEMMA_CONTRADICTION_DETECTOR,
    GEMMA_RELEVANCE_EVALUATOR,
    GEMMA_FACT_STITCHING,
    GEMMA_CONVERSATIONALITY_EVAL_FOR_STITCHING,
]